...
def generate_random_time_series(length=100, lag=2):
    ...
    # Initialize x1 with same size as x0
    x1 = np.zeros(length)
    
    # Introduce a lagged relationship from x0 to x1
    # For the first 'lag' elements, fill x1 with noise or another baseline since x0 doesn't have prior values for them
    x1[:lag] = np.random.normal(0, 1, lag)  # Optionally, could be set to other values or derived from x0 differently
    
    # Fill in the rest of x1 with values from x0 lagged by 'lag' steps plus some noise
    for i in range(lag, length):
        x1[i] = x0[i - lag] + np.random.normal(0, 1)  # Adding noise to introduce variability
...
class TestTimeseriesTests(unittest.TestCase):
    def setUp(self):
        ...
        self.mean_reverting_series = pd.Series(TimeseriesTests.generate_mean_reverting_series())
        self.trending_series = pd.Series(TimeseriesTests.generate_trending_series())
        self.random_walk_series = pd.Series(TimeseriesTests.generate_random_walk_series(n=2000, start_value=0, step_std=1))
...
# TODO : Finish below tests
    ...
    def test_hurst_exp_trending_series(self):
        self.trending_series = pd.Series(TimeseriesTests.generate_trending_series())
        print(self.trending_series)
        x = np.arange(1, 101)
        data = 2 * x + 5  # Linear trend: change the coefficients as needed
    ...
    #     data = generate_random_time_series(lag=5)
    ...
    #     expected_lag = 5
    #     selected_lag = TimeseriesTests.select_lag_length(data, maxlags=10, criterion='aic')
