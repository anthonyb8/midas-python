...
class DataApp(EWrapper, EClient):
    ...
    def connectAck(self):
        super().connectAck()
        self.event_logger.info('Established Data Connection')
        self.connected_event.set()

    #### wrapper function for disconnect() -> Signals disconnection.
    def connectionClosed(self):
        super().connectionClosed()
        self.event_logger.info('Closed Data Connection.')
    ...
    def nextValidId(self, orderId):
        ...
        self.event_logger.info(f"Next Valid Id {self.next_valid_order_id}")
    ...
    def realtimeBar(self, reqId: int, time: int, open: float, high: float, low: float, close: float, volume: int, wap: float, count: int):
        ...
        self.current_bar_data[contract] = new_bar_entry
