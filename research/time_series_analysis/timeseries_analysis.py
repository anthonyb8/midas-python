...
class TimeseriesTests:
    ...
    @staticmethod
    def display_adf_results(adf_results:dict):
        ...
        print(f"\n{adf_df}")
        print("** IF p-value < 0.05 and/or statistic < statistic @ confidence interval, then REJECT the Null that the time series posses a unit root (non-stationary).\n")
    ...
    @staticmethod
    def display_kpss_results(kpss_results:dict):
        ...
        print(f"\n{kpss_df}")
        print("** IF KPSS statistic > statistic @ confidence interval, then reject the NUll that time-series is stationary.\n")
    ...
    @staticmethod
    def display_pp_results(pp_results:dict):
        ...
        print(f"\n{pp_df}")
        print("** IF p-value < 0.05, then REJECT the Null Hypothesis of a unit root (non-stationary time series).\n")
    ...
    @staticmethod
    def display_johansen_results(johansen_results:dict, num_cointegrations:int):
        ...
        print(johansen_df)
        print(f"** IF Trace Statistic > Critical Value AND Max Eigenvalue > Critical Value then Reject Null of at most r cointegrating relationships.(r=0 in first test)\n")
    ...
    @staticmethod 
    def display_durbin_watson_results(dw_results:dict):
        ...
        print(f"\n{dw_df}")
        print("** If the Durbin-Watson statistic is significantly different from 2 (either much less than 2 or much greater than 2), it suggests the presence of autocorrelation in the residuals.\n")
    ...
    @staticmethod
    def display_ljung_box_results(ljung_box_results:dict):
        ...
        print(f"\n{ljung_box_df}")
        print("** IF p-value < 0.05, then REJECT the Null Hypothesis of no autocorrelation (i.e., autocorrelation is present).\n")
    ...
    @staticmethod
    def display_shapiro_wilk_results(sw_results:dict):
        ...
        print(f"\n{sw_df}")
        print("** If p-value < 0.05, then REJECT the Null Hypothesis of normality (i.e., data is not normally distributed).\n")
    ...
    @staticmethod
    def display_breusch_pagan_results(bp_results:dict):
        ...
        print(f"\n{bp_df}")
        print("** IF p-value < 0.05, then REJECT the Null Hypothesis of homoscedasticity (constant variance) in favor of heteroscedasticity (varying variance).\n")
    ...
    @staticmethod
    def display_white_test_results(white_results:dict):
    ...
    @staticmethod
    def display_granger_results(granger_results:dict):
        # Creating DataFrame from the results
        ...
        print(f"\n{granger_df}")
        print("** IF p-value < significance level then REJECT the NUll and conclude that the lagged values of one time series can provide useful information for predicting the other time series. \n")
    ...
    # -- Forescast Metrics -- 
    @staticmethod
    def evaluate_forecast(actual:pd.DataFrame, forecast:pd.DataFrame):
        """
    ...
    @staticmethod
    def plot_price_and_spread(price_data:pd.DataFrame, spread:pd.Series):
        """
