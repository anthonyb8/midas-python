...
class SummaryStats:
    ...
    def _total_winning_trades(self):
        return self.trade_log[self.trade_log['net_gain/loss'] > 0]

    def _total_lossing_trades(self):
        return self.trade_log[self.trade_log['net_gain/loss'] < 0]
    
    def _avg_win_percent(self):
        return self.total_winning_trades['gain/loss (%)'].mean()

    def _avg_loss_percent(self):
        return self.total_losing_trades['gain/loss (%)'].mean()

    def _compute_gross_profits(self):
        return self.total_winning_trades['net_gain/loss'].sum()

    def _compute_gross_losses(self):
        return abs(self.total_losing_trades['net_gain/loss'].sum())
    ...
    def _ending_equity(self):
        return self.equity_log['equity_value'].iloc[-1]
    
    def _max_drawdown(self):
        """Calculate maximum drawdown."""
        return self.equity_log['percent_drawdown'].min()
    
    def total_return(self):
        """Calculate total return from the start to the end of the period."""
        return ((self.ending_equity / self.equity_log['equity_value'].iloc[0]) - 1) * 100
    ...
    def annual_standard_deviation(self):
        ...
        return daily_std_dev * (252**0.5)  # Assuming 252 trading days in a year
    ...
    def profit_and_loss_ratio(self):
        ...
        if self.avg_loss_percent == 0:
            return float('inf')
        return self.avg_win_percent / abs(self.avg_loss_percent)
