...
class TimeseriesTests:
    ...
    @staticmethod
    def generate_trending_series(n=2000, start_value=0, trend=1, step_std=5):
        """
    ...
    @staticmethod
    def select_lag_length(data: pd.DataFrame, maxlags: int = 10, criterion: str = 'aic') -> int:
        """
    ...
    def hurst_exponent(series: np.array):
        ...
        for lag in lags:
            ...
            lagged_diff = np.subtract(series[lag:], series[:-lag])
            
            # Calculate standard deviation of lagged differences
            std_lagged_diff = np.sqrt(np.std(lagged_diff))
